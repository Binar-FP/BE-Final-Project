...
const refreshKey = process.env.REFRESH_TOKEN_TICKET || "This is a secret key";
  ...
  const signin = async (req, res) => {
    ...
    const { id,countryCode, username, email, password, NIK, address, phoneNumber, imageUrl, roleId, gender, dateOfBirth } = user;
      ...
      const refresh = await User.update({refresh_token: refreshToken},{
        where:{id: id}
      });
