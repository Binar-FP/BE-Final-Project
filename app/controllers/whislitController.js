const { whislit, Flight, WhislistDestination} = require("../models")

const addWhislist = async (req, res) => {
  try {
    const { userId, destinationId} = req.body

    const namewhislist = await whislit.findOne({
      where: {
    ...
    const newWhislist = await whislit.create({
      userId,
      destinationId,
    })
...
const getWhislistUser = async (req, res) => {
  ...
  try {
    const dataWhislist = await whislit.findAll({
      where: {
        userId: userId,
      },
      order: [["id", "DESC",],],
